...
@click.option('--patch_sz',      help='Patch size during training', metavar='INT',                  type=int,default=None,show_default=True)
@click.option('--n_patches',     help='Number of patches extracted from each image', metavar='INT', type=int,default=None,show_default=True)
@click.option('--cbase',         help='Channel multiplier  [default: varies]', metavar='INT',       type=int)
@click.option('--cres',          help='Channels per resolution  [default: varies]', metavar='LIST', type=parse_int_list)
@click.option('--ares',          help='Resolutions with self-attention [default: varies]', metavar='LIST', type=parse_int_list)
...
@click.option('--weight_n',      help='Weight given to noise patches', metavar='FLOAT',             type=click.FloatRange(min=0, min_open=True), default=1.0, show_default=True)
...
@click.option('--snap',          help='How often to save snapshots', metavar='TICKS',               type=click.IntRange(min=1), default=8, show_default=True)
@click.option('--dump',          help='How often to dump state', metavar='TICKS',                   type=click.IntRange(min=1), default=8, show_default=True)
...
def main(**kwargs):
    ...
    if opts.ares is not None:
        c.network_kwargs.attn_resolutions  = opts.ares
    ...
    c.update(patch_sz=opts.patch_sz)
    c.update(n_patches=opts.n_patches)
    ...
    dist.print0(f'PFGM++:                  {opts.pfgmpp}')
