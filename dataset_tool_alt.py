...
def open_image_folder(source_dir, *, max_images: Optional[int]):
    input_images = [str(f) for f in sorted(Path(source_dir).rglob('*')) if is_npy_ext(f) and os.path.isfile(f)]
...
def main(
    ...
    for idx, image in tqdm(enumerate(input_iter), total=num_files):
        ...
        img = image['img']
        #img = transform_image(image['img']) # breaks exakt [0,1]
        #print(np.min(img))
        #print(np.max(img))
        if img is None:
            continue
        ...
        save_bytes(os.path.join(archive_root_dir, archive_fname),img.tobytes())
